 ...
 * S10 以降は順次追加。server-only（機密本文を扱う。ログに出さない＝ガードレール7）。
 */

/** どの実行パスへ振り分けるか（current_step 基準: <3=解析, →S4, →S6, →S8, それ以降=未実装）。 */
export function nextStepToRun(
  currentStep: number | null | undefined,
): "analysis" | 4 | 6 | 8 | "unsupported" {
  ...
  return "unsupported"; // 9+ : S10 以降は未実装
}
...
/** 指定ステップ（4=妥当性評価 / 6=応答方針 / 8=代表補正）を実行する。 */
