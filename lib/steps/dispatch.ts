 ...
 * 後続スライス。いまは「送信＝次の 1 ステップを実行」。実装済みは S4(妥当性)/S6(応答方針) のみ、
 * S8 以降は unsupported を返す。server-only（機密本文を扱う。ログに出さない＝ガードレール7）。
 */

/** どの実行パスへ振り分けるか（current_step 基準: <3=解析, 3-4→S4, 5-6→S6, 7+=未実装）。 */
export function nextStepToRun(
  currentStep: number | null | undefined,
): "analysis" | 4 | 6 | "unsupported" {
  const cs = currentStep ?? 0;
  if (cs < 3) return "analysis";
  if (cs <= 4) return 4; // 解析済（3）→ 妥当性評価
  if (cs <= 6) return 6; // S4-5 済（5）→ 応答方針
  return "unsupported"; // 7+ : S8 以降は未実装
}
