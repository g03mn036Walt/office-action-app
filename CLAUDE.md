...
## UI / デザイン
- 視覚設計の正は **Claude Design**（claude.ai/design のデザインシステム）。`/design-sync`（DesignSync）で `components/ui/` に同期し、画面はそのコンポーネントを使って実装する（PRD §9.0）。
- 同期は1コンポーネントずつ（全置換しない）。場当たりのスタイルを増やさず、デザインシステム由来のトークン/コンポーネントに従う。
