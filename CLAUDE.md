...
## UI / デザイン
- 視覚設計の正は **Claude Design**（claude.ai/design）。本アプリのデザインは通常の「デザイン」プロジェクト **`office-action-app-ui`**（projectId `0e0eabb0-9f93-49db-a071-619cc988ef28`、種別は通常デザインで `list_projects` には出ない）に置く。空の "Design System" プロジェクトとは別物。
- 取り込み手順: claude.ai/design 側で各画面の **「Hand off the project to Claude Code」** を実行 → 提示される projectId とファイル名（例 `Login.dc.html`）を使い、DesignSync（`/design-sync`）で `get_file` して読む。`.dc.html` は Claude Design のプレビュー形式なので、**React/Tailwind に翻訳して `components/ui/` に実装**する（生ファイルは実行時には使わない）。
- 同期は1画面/1コンポーネントずつ（全置換しない）。デザイントークン（カラー/フォント）は **`app/globals.css` の `@theme`** に定義済み。画面はこのトークンと `components/ui/`（例 `Brandmark`）から組み、場当たりのスタイルを足さない（PRD §9.0）。
