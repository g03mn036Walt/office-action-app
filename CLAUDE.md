...
- アプリ本体は標準的な Next.js 構成（`app/`, `components/`, `lib/` 等）。`lib/` に Supabase クライアントと Claude 呼び出しのサーバー側ラッパを置く想定。

## コマンド（プロジェクト作成後に確定・更新する）
- 開発サーバー: `npm run dev`
- ビルド: `npm run build`
- Lint: `npm run lint`
> 上記は雛形。実際のスクリプト名が決まったら本セクションを更新すること。

## 環境変数（サーバー専用。`.env.local` / Vercel に設定）
- `ANTHROPIC_API_KEY`（**NEXT_PUBLIC を付けない**）
