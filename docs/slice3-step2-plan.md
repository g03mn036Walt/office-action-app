...
## 後続 2b（実装済み・設計の正は `docs/slice3-step2b-plan.md`）
- vision 経路の大部スキャンは **1頁ずつ分割して並列転写**する方式で実装済み。当初「引用文献1は1.5万トークンで通るため緊急度低」としていたが、実測では単一 vision 呼び出しが (1) `maxDuration=60s` を超過し、(2) 密頁を黙って圧縮して取りこぼしていた（引用文献1: 単一≈59,715字 ↔ 1頁分割≈105,191字）＝速度・正確性の両面で 2b が必要だった。
- 本番 `maxDuration`: Hobby は上限 60s で **300 への引き上げは不可**。`maxDuration=60` 据え置きのまま、1頁分割の並列転写（実測 ~46s）＋要約は再送1回で吸収する設計に確定。
